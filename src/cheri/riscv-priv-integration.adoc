...
The address field follows the standard `mepc` rules:
...
The address observable through <<mepc_y>> therefore depends on IALIGN and is not always the value that was written (similar to the MODE field of <<mtvec_y>>).
Software must ensure that every observable address is <<section_cap_representable_check, represented exactly>>, otherwise the hardware sets the written {ctag} to zero.

Sealed capabilities may be written to <<mepc_y>>.
However, changing the address of a sealed capability invalidates it (see <<SCADDR>>), so the {ctag} is set to zero on writing if `mepc.address[0]=1`, or if `mepc.address[1]=1` when IALIGN is fixed to 32.
When IALIGN is programmable, `mepc.address[1]` is only masked on reading.
A sealed capability written with `mepc.address[1]=1` is therefore read back with its {ctag} set while IALIGN=16, but with its {ctag} cleared while IALIGN=32.

When a trap is taken into M-mode, the <<pcc>> is written to <<mepc_y>>, with the address following the standard `mepc` behavior.
...
Following the standard `mepc` rules, <<mepc_y>> need not be able to hold all possible <<section_invalid_addr_conv,invalid addresses>>.
